...
window.applyGuidePatches = function applyGuidePatchesWithSmartHomeAndPool() {
  ...
  if (pool && !pool.html.includes('Current pool automation')) {
    pool.html = pool.html.replace(
      ...
      '<p><b>WYBOT:</b> the S3 is currently scheduled to clean <b>daily at midnight</b>, with a mix of cleaning types/modes programmed across the week. Cleaning cycles finish before the 2:00 AM pump schedule begins.</p>' +
      '</div>'
    );
  }
};
